...
// One named route per manifest endpoint (`polaris.auth.login`, ...) under path_prefix, all served by
// the controller, which runs the whole Polaris middleware stack and handler.
$prefix = rtrim((string) $polaris['path_prefix'], '/');
$manifest = (new Loader($polaris['manifest_directory'] ?? Loader::defaultDirectory()))->load();
$routes = [];
